...
@section('content')
    <div class="py-8 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <x-breadcrumb :links="['Manajemen Kategori' => route('admin.dashboard.category'), 'Tambah Baru' => '#']" />

        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-gray-50 p-6 sm:px-8 border-b border-gray-100">
                <h3 class="text-xl font-extrabold text-gray-800">Tambah Kategori Lomba</h3>
                <p class="text-sm text-gray-500 mt-1">Buat kategori baru untuk mengelompokkan jenis perlombaan (Misal: TK,
                    SD, SMP).</p>
            </div>

            <form action="{{ route('admin.dashboard.category.store') }}" method="POST" class="p-6 sm:p-8">
                @csrf

                <div class="mb-6">
                    <x-input-label for="name" :value="__('Nama Kategori')" class="font-bold text-gray-700 mb-2 block" />
                    <x-text-input type="text" id="name" name="name"
                        ...
                        placeholder="Contoh: Kategori TK/RA" value="{{ old('name') }}" required autofocus />
                    @error('name')
